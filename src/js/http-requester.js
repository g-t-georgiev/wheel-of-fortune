...
/**
 * Represents the header configuration options for an HTTP request. Instances are immutable. 
...
class HttpHeaders {
     ...
     * Represents the header configuration options for an HTTP request. Instances are immutable. 
     * Modifying methods return a cloned instance with the change. The original object is never changed.
     * @param {string | { [header: string]: string | string[] } | Headers} [headers] 
     */
    constructor(headers) {
        if (headers && typeof headers === 'object') {
            ...
            Object.entries(parseResponseHeaders(headers)).forEach(([ k, v ]) => {
                this.#headers.set(k, v);
            });
        }
    }
     ...
     * @returns {string | null} The value string if the header exists, null otherwise
     */
    get(name) {
        if (this.has(name)) {
            let header = this.#headers.get(name);
     ...
     * @returns {string[]} A list of header names.
     */
    keys() {
     ...
     * @returns {string[] | null} A string of values if the header exists, null otherwise.
     */
    getAll(name) {
        if (this.has(name)) {
            let header = this.#headers.get(name);
     ...
     * @param {string} name The header name for which to append the values.
     * @param {string | string[]} value The value to append.
     * @returns {HttpHeaders} A clone of the HTTP headers object with the value appended to the given header.
     */
    append(name, value) {
        value = !Array.isArray(value) ? [ value ] : value;

        const headers = [ ...this.#headers.entries() ].reduce((o, [ k, v ]) => ({ ...o,  [k]: v }), {});
        let current;
        ...
        current = !Array.isArray(headers[name]) ? [ headers[name] ] : headers[name];
        current.push(...value);
        headers[name] = Array.from(new Set(current));
        current = headers[name];

        if (current.length === 1) {
            headers[name] = current.pop();
        }
     ...
     * If the header already exists, its value is replaced with the given value in the returned object.
     * @param {string} name The header name.
     * @param {string | string[]} value The value or values to set or override for the given header.
     * @returns {HttpHeaders} A clone of the HTTP headers object with the newly set header value.
     */
    set(name, value) {
        const headers = [ ...this.#headers.entries() ].reduce((o, [ k, v ]) => ({ ...o,  [k]: v }), {});
     ...
     * The value or values to delete for the given header. Optional. Default is `undefined`.
     * @returns {HttpHeaders} A clone of the HTTP headers object with the given value deleted.
     */
    delete(name, value) {
        const headers = [ ...this.#headers.entries() ].reduce((o, [ k, v ]) => ({ ...o,  [k]: v }), {});
        ...
        if (this.has(name)) {
            if (value) {
                value = !Array.isArray(value) ? [ value ] : value;
                let current = !Array.isArray(headers[name]) ? [ headers[name] ] : headers[name];

                value.forEach(v => {
                    let index = current.findIndex(c => c === v);
    ...
    [Symbol.toPrimitive](hint) {
        return [ ...this.#headers.entries() ].map(([ k, v ]) => `${k}: ${Array.isArray(v) ? v.join('; '): v}`).join('\r\n');
    }
}
